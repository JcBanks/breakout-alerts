...
def generate_analysis(ticker_data, symbol):
    ...
    nl = '\n'
    symbol = "$"+symbol
    st.text(f"Upside Breakout Alert: {symbol} just hit a new 1-month high of ${price:,.2f}.")
    st.text(f"{symbol} is a ${market_cap:,.2f} billion market cap member of the {industry} industry group.")
    st.text(f"This marks the {signal_count}{suffix} upside breakout for {symbol} over the last 21 trading days.")
